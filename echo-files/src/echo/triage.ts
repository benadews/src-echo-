  ...
  try {
    ...
    for (const channel of channels) {
      ...
      for (const convo of conversations) {
        ...
        const verdict = teamworkMatch ? assessStaleness(teamworkMatch) : 'no_task_exists'
...
function assessStaleness(match: TaskCandidate): 'task_current' | 'task_stale' {
  const daysSince = (Date.now() - new Date(match.updatedAt).getTime()) / 86_400_000
  return daysSince > 5 ? 'task_stale' : 'task_current' // placeholder threshold — see below
}
